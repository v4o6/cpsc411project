...
//		};
	...
	public TRExp visit(FunctionDeclaration n) {
		...
		frame = newFrame(label, n.params.size());
		FunTable<Access> oldEnv = currentEnv;
		currentEnv = FunTable.theEmpty();

		n.params.accept(this);
		currentEnv = currentEnv.merge(oldEnv);

		TRExp statements = n.statements.accept(this);
		TRExp returnValue = n.value.accept(this);
		IRStm body = IR.SEQ(
				statements.unNx(),
				// this is the location of the error???
				IR.MOVE(frame.RV(), returnValue.unEx())
		);
		body = frame.procEntryExit1(body);
		frags.add(new ProcFragment(frame, body));
	...
	public TRExp visit(FormalList ns) {
		for (int i=0; i<ns.size(); i++){
			Access param = frame.getFormal(i);
			putEnv(ns.elementAt(i).name, param);
		}
		return null;
	}

	@Override
	public TRExp visit(Parameter n) {
		throw new Error("Not used");
	}

	@Override
	public TRExp visit(FunctionCall n) {
		Label function = Label.get(n.identifier);
		IRExp[] args = new IRExp[n.args.size()];
		for (int i = 0; i < n.args.size(); i++) {
			args[i] = n.args.elementAt(i).accept(this).unEx();
		}
		return new Ex(IR.CALL(function, args));
	}
	...
	@Override
	public TRExp visit(LambdaType n) {
		throw new Error("Not implemented");
	}
}
